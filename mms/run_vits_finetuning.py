...
def main():
    ...
    def _get_storage_options():
        opts = {}
        aws_access_key_id = data_args.aws_access_key_id or os.environ.get("AWS_ACCESS_KEY_ID")
        aws_secret_access_key = data_args.aws_secret_access_key or os.environ.get("AWS_SECRET_ACCESS_KEY")
        aws_endpoint_url = data_args.aws_endpoint_url or os.environ.get("AWS_ENDPOINT_URL")
    ...
    def _is_s3_or_local(path):
        return path.startswith("s3://") or os.path.exists(path)

    storage_options = _get_storage_options()

    if training_args.do_train:
        if _is_s3_or_local(data_args.dataset_name):
            raw_datasets["train"] = datasets.load_from_disk(
                data_args.dataset_name,
                storage_options=storage_options,
            )
            if data_args.train_split_name in raw_datasets["train"]:
                raw_datasets["train"] = raw_datasets["train"][data_args.train_split_name]
        else:
    ...
    if training_args.do_eval:
        if _is_s3_or_local(data_args.dataset_name):
            raw_datasets["eval"] = datasets.load_from_disk(
                data_args.dataset_name,
                storage_options=storage_options,
            )
            if data_args.eval_split_name in raw_datasets["eval"]:
                raw_datasets["eval"] = raw_datasets["eval"][data_args.eval_split_name]
        else:
