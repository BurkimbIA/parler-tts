...
def create_xtts_trainer_parser():
    ...
    parser.add_argument(
        "--max_text_length", type=int, default=400, help="Max text length"
    )
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Auto-resume from latest checkpoint if available"
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
...
def find_latest_checkpoint(output_path):
    ...
    # Look for checkpoints in all subdirectories of output_path
    checkpoint_pattern = os.path.join(output_path, "**", "checkpoint_*.pth")
    checkpoints = glob.glob(checkpoint_pattern, recursive=True)
    
    if checkpoints:
        # Filter out config files and only keep actual checkpoint files
        actual_checkpoints = [cp for cp in checkpoints if not cp.endswith('config.json')]
        
        if actual_checkpoints:
            # Sort by modification time (newest first)
            sorted_checkpoints = sorted(actual_checkpoints, key=os.path.getmtime, reverse=True)
            
            # Try each checkpoint to find a valid one
            for checkpoint_path in sorted_checkpoints:
                try:
                    ...
                    print(f" > Checkpoint {checkpoint_path} is corrupted: {e}")
                    continue
            
            print(" > All found checkpoints are corrupted")
...
def train_gpt(
    ...
    if checkpoint_path:
        ...
        print(f" > Using specified checkpoint: {resume_checkpoint}")
    elif resume:
        # Auto-find latest valid checkpoint
        resume_checkpoint = find_latest_checkpoint(output_path)
    ...
    trainer = Trainer(
        TrainerArgs(
            restore_path=resume_checkpoint,  # Use found checkpoint or None for fresh start
